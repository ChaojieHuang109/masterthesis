...
remove(fit_chlratio1,fit_chlratio2,fit_chlratio3,fit_chlratio4)
remove(fit_sys1,fit_sys2,fit_sys3,fit_sys4)
remove(Jointfit1,Jointfit2,Jointfit3,Jointfit4,Jointfit5,Jointfit6,Jointfit7,Jointfit8,Jointfit9,Jointfit10)
...
marginal_df <- summaries$marginal %>%
  rename("m.DIC"=DIC,"m.LPML"=LPML,"m.WAIC"=WAIC)
conditional_df <- summaries$conditional %>%
  rename("c.DIC"=DIC,"c.LPML"=LPML,"c.WAIC"=WAIC)

Fitness_df <- cbind(marginal_df,conditional_df)

write.csv(Fitness_df,"/ANALYSE_AREA/P_INTG01/Chaojie/code_new/RData/Fitness_df.csv")
...
values_list_s <- read.csv("/ANALYSE_AREA/P_INTG01/Chaojie/code_new/RData_s/values_s.csv")
